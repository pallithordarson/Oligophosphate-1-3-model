...
oldstart=start1; % saves a spare copy of initial guess(ss) for future reference
...
%or number of proton resonances in NMR used. Can be 1 = local fitting but
%as many as >600,e.g. if the whole UV-Vis spectra from 200-800 nm with 1 nm
%resolution is used (WARNING - if large segment of the UV-VIS remain = 0 throughout the titration they should not be included).
...
UDAL=DA-hostA; %substracts the initial raw data value from exp no 1 (row 1) from all the raw data points.
%after this the first row should read as = 0 0 0.... This means that UDAL
%is now = DeltaA (UV) or Deltadelta (NMR) or DeltaY (general) as per eq. 14
%and 15 in P.Thordarson, Chem. Soc. Rev., 2011, Vol 40, p 1305-1323.
%This means we are only fitting the change in physical property  Y.
DA=UDAL; %Reassigns the raw data matrix DA = UDAL (the one with the first row of data subtracted).
...
[results, fval, exitflag, output] = fminsearch('nmr1to3fitbindnoncoopkb',start1,options,initial,DA);
...
%NEXTLINEVARIABLE (Marker for section that is variable between different fitting programs)   
[ss, EA, HG, HG2, HG3, b, RR]=nmr1to3fitbindnoncoopkb(para,initial,DA);
...
xx=Ltot./htot; %Calculates the number of Guest added as xx = Total Guest(Ltot) /Total Host (htot) concentration.
...
Fitresults=struct('K1',results,'Initial_guess_K1',oldstart,'Least_sq_error_Spectra_Percent',perEA);
...
FitConcAndSpectra=struct('FreeHost',b,'Host_Guest_Complex',[HGG],'Spectra_Host',hostA(1,:),'Spectra_HostGuest',EA,'fitted',CA);
...
FitStats=struct('SumofSq_ss',ss,'StandardError_Y_estim_SE',SE,'Chi_squared',chiSQ,'Covariance_Fit_covfit',covFit,...
    'maxlikel',maxloglik,'BIC',myBIC,'sigma',sigmas,'DegreesFreedom',deFree,'Program_Used','fitbinding1to2nmr');
%ENDVARIABLE (Marker for end of section that is variable between different fitting programs)
...
save ('fitted1to3noncoopp','Fitallstuff','-append');
%end of program.